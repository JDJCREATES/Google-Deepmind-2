...
export function useChatLogic({ electronProjectPath }: UseChatLogicProps) {
  ...
  const handleSendMessage = async (overridePrompt?: string) => {
    ...
    // Clear input immediately if not an override (or just always clear)
    if (!overridePrompt) {
        setInputValue('');
    }
        ...
        // ... (existing stream logic) ...
         ...
         // Run Complete - Capture screenshot and create preview
           ...
           if (targetRunId && (window as any).electron) {
              // ... existing screenshot logic (omitted for brevity, keep existing) ...
              // RE-INLINE IT TO BE SAFE
             ...
             (window as any).electron.createRunPreview(targetRunId, finalProjectPath)
               .then(() => {
                 setTimeout(() => {
                   (window as any).electron.captureRunScreenshot(targetRunId, 'complete', 'Agent run completed successfully');
                 }, 2000);
               })
               .catch(console.warn);
           }
  ...
  const handleMessageEdit = useCallback((messageId: string, newContent: string) => {
    ...
    // 1. Truncate run history (delete edited message + future)
    truncateRunMessages(activeRunId, messageId, false);
    // 2. Resend new content as prompt
    handleSendMessage(newContent);
  }, [activeRunId, truncateRunMessages]);

  const handleMessageRewind = useCallback((messageId: string) => {
      if (!activeRunId) return;
      // Truncate future (keep this message)
      truncateRunMessages(activeRunId, messageId, true);
      // Don't auto-send; user has rewound to this state
  }, [activeRunId, truncateRunMessages]);
